...
public class RoomImageController {
    ...
    public Resp<?> getByRoom(
            @PathVariable Integer roomId
    ) {
        List<RoomImageDTO> images = service
                .getByRoom(roomId)
                .stream()
                .map(this::toDTO)
                .toList();
    ...
    public Resp<?> add(
        ...
        if (dto.getRoomId() == null
                || !isValidUrl(dto.getImageUrl())) {

            return Resp.error(
                    "Room id and a valid HTTP/HTTPS image URL are required"
            );
        }

        Room room = roomService
                .getRoomEntityById(dto.getRoomId());

        if (!canManage(room)) {
            return Resp.error(
                    "You can add images only to your own rooms"
            );
        }
    ...
    public Resp<?> update(
        ...
        if (!canManage(image.getRoom())) {
            return Resp.error(
                    "You can update images only for your own rooms"
            );
        }

        if (!isValidUrl(dto.getImageUrl())) {
            return Resp.error(
                    "A valid HTTP/HTTPS image URL is required"
            );
        }
    ...
    public Resp<?> delete(
        ...
        if (!canManage(image.getRoom())) {
            return Resp.error(
                    "You can delete images only from your own rooms"
            );
        }
